...
def analyze_user_message(user_message):
    ...
    system_prompt = (
        ...
        "КРИТИЧЕСКОЕ ТРЕБОВАНИЕ ПО ИСПРАВЛЕНИЮ ОШИБОК:\n"
        "Пользователь может допускать опечатки (например, 'Нарафен', 'Осперин') или голосовой ввод может разделить слово пробелами (например, 'нура фен', 'парацета мол'). "
        "Ты должен ОБЯЗАТЕЛЬНО исправить эти ошибки, объединить разделенные слова и записать в поле 'drug_name' СТРОГО ПРАВИЛЬНОЕ, ОЧИЩЕННОЕ название препарата в нижнем регистре (например: 'нурофен', 'аспирин', 'парацетамол').\n\n"
